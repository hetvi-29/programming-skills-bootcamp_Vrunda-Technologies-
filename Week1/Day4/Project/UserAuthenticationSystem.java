...
class Authentication {
    ...
    String storageFilePath = "E:\\programming-skills-bootcamp\\Week1\\Day4\\Project\\userData.txt";
    ...
    public void login() {
        ...
        if (email.contains("@") && email.contains(".")) {
            ...
            if (isUserExists(email)) {
                // Checking if the password matches the password associated with the email in
                // the hashmap
    ...
    // Creating a method to read data(eg. email,password) from the file and adding
    // retrieved data into the userMap hashmap.
    public void read() {
        userMap.clear();// Clearing the HashMap object before reading the file data
        try {
            // Creating an object of FileInputStream class to read a file from a specific
            // file path
            ...
            // In simpler words, the above lines of code will help us to read a file from a
            // specific path and convert the content from bytes to characters so that we can
            // easily read it.
            String line;
        ...
        do {
            ...
            switch (choice) {
                case 1:
                    ...
                    auth.read();// After registering a new user, this line reads the user data from the file and
                                // stores it in the userMap object.
                    break;
                case 2:
                    auth.read();// This line reads the user data from the file and stores it in the userMap
                                // object.
                    auth.login();
                    break;
                case 3:
